...
def uebersicht(theard_nr ,tripnummer):
...
def Programmauswahl(programm):
    ...
    if programm =="trips":
        ...
        uebersichttrip_pro= int(lasttrip(uebersichts_tabelle)+1)
        liste_der_trips = trip_handler()
        for i in range (int(prozess.get())):
            threading.Thread()
            prozesse.append(threading.Thread(target=uebersicht, args=(i,liste_der_trips[i])))
            prozesse[i].start()
            time.sleep(0.2)
    else:
